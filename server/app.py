...
class Memos(Resource):
    def get(self):
        try:
            ...
            if company:
                # If 'company' is provided, filter by that company
                memos = Memo.query.filter_by(company=company).all()
                logger.info(f"Memos retrieved for company: {company}")
            else:
                # No 'company' param => retrieve ALL memos
                memos = Memo.query.all()
                logger.info("All memos retrieved.")
...
class Invoices(Resource):
    def get(self):
        try:
            ...
            if company:
                # If a 'company' param is provided, filter by that
                invoices = Invoice.query.filter_by(company=company).all()
                logger.info(f"Invoices retrieved for company: {company}")
            else:
                # No 'company' param => fetch all invoices
                invoices = Invoice.query.all()
                logger.info("All invoices retrieved.")
...
# Optional "future" routes + categories, user companies, etc. remain the same
